...
<table border="0" cellspacing="1" cellpadding="4" width="100%">
    <tr align="center" bgcolor="<?php echo $config['vdarkborder']; ?>" class="white">
        ...
        <td><span style="color: white"><b>banned in</b></span></td>
    </tr>
<?php
$i = 0;
$next_page = false;
foreach($bans as $ban)
{
    ...
    // Verificar si el valor de expires es válido
    $expires = $ban['expires_at'];
    if ($expires == "-1") {
        $expires_text = 'Never';
    } else if ($expires == "0" || $expires == null) {
        $expires_text = 'Unknown expiration';
    } else {
        $expires_text = date("H:i:s", $expires) . '<br/>' . date("d M Y", $expires);
    }
?>
    <tr align="center" bgcolor="<?php echo getStyle($i); ?>">
        <td height="50" width="140"><?php echo getPlayerLink(getPlayerNameByAccount($ban['account_id'])); ?></td>
        <td><?php echo getBanType($ban['type']); ?></td>
        <td><?php echo $expires_text; ?></td>
...
function getPlayerNameByAccount($accountId)
{
    global $db;
    if(is_numeric($accountId))
    {
        // Obtener el jugador baneado de la cuenta
        $playerQuery = $db->query('SELECT `banned_player` FROM `account_bans` WHERE `account_id` = ' . $accountId . ' ORDER BY `banned_at` DESC LIMIT 1;')->fetch();
        if($playerQuery)
        {
            return $playerQuery['banned_player'];
        }
        else
        {
            return "Unknown name";
