<style>
    ...
    .bg-danger {
        background-color: #FF183E !important;
    }
    ...
    .bg-success {
        background-color: #2EEB59 !important;
    }
...
WHERE killers.death_id = '" . $death['id'] . "' ORDER BY killers.final_hit DESC, killers.id ASC")->fetchAll();
        ...
        $deaths_db = $db->query('SELECT
				...
				FROM `player_deaths`
				WHERE `player_id` = ' . $player->getId() . ' ORDER BY `time` DESC LIMIT 50;')->fetchAll();
