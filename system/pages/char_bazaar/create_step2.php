...
if (isset($_POST['auction_submit']) && isset($_POST['auction_character'])) {
    ...
    if ($frags_enabled) {
        $query = $db->query(
            ...
            'FROM `player_killers` ' .
            'WHERE `player_id` = ' . (int) $getCharacter['id'] . ' ' .
            'GROUP BY `player_id` ' .
            'ORDER BY COUNT(`player_id`) DESC');

        $frags = $query ? $query->fetch(PDO::FETCH_ASSOC) : null;
        $frags_count = (int) ($frags['frags'] ?? 0);
    }
    if ($frags_count == 0) {
    ...
    /* GET COINS */
    $getCoins = $db->query('SELECT `coins_transferable` FROM `accounts` WHERE `id` = ' . $getCharacter['account_id']);
    $getCoins = $getCoins->fetch();
    $availableCoins = (int)($getCoins['coins_transferable'] ?? 0);

    if ($availableCoins >= $charbazaar_create) {
    ...
    /* GET HOUSE */
    $getHouse = $db->query('SELECT `owner` FROM `houses` WHERE `owner` = ' . (int) $getCharacter['id']);
    ...
    /* GET GUILD */
    $getGuildOwner = $db->query('SELECT `ownerid` FROM `guilds` WHERE `ownerid` = ' . (int) $getCharacter['id']);
    $getGuildOwner = $getGuildOwner->fetch();

    $getGuildInvited = $db->query('SELECT `player_id` FROM `guild_invites` WHERE `player_id` = ' . (int) $getCharacter['id']);
    $getGuildInvited = $getGuildInvited->fetch();

    $getGuildMember = $db->query('SELECT `player_id` FROM `guild_membership` WHERE `player_id` = ' . (int) $getCharacter['id']);
    ...
    /* GET MARKET */
    $getMarket = $db->query('SELECT `player_id` FROM `market_offers` WHERE `player_id` = ' . (int) $getCharacter['id']);
    ...
    $getOnline = $db->query('SELECT `player_id` FROM `players_online` WHERE `player_id` = ' . (int) $getCharacter['id']);
