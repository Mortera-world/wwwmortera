                                    ...
                                    <table class="TableContent" width="100%" style="border:1px solid #faf0d7;">
                                        ...
                                        if ($logged) {
                                            ...
                                            foreach ($getBidsbyAccount as $BidbyAccount) {
                                                ...
                                                $getAuctionbyBid = $db->query("SELECT `id`, `account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`, `bid_account`, `bid_price`, `status` FROM `myaac_charbazaar` WHERE `id` = {$BidbyAccount['auction_id']}");
                                                ...
                                                $auctionEndTimestamp = strtotime($getAuctionbyBid['date_end'] ?? '');
                                                $isAuctionFinished = $auctionEndTimestamp !== false && $auctionEndTimestamp <= time();
                                                $isWinner = isset($getAuctionbyBid['bid_account']) && (int) $getAuctionbyBid['bid_account'] === (int) $account_logged->getId();
                                                $isDelivered = $isWinner && (int) $getAuctionbyBid['status'] === 1;

                                                $statusColor = 'inherit';
                                                $statusTitle = $isAuctionFinished ? 'Auction lost' : 'Outbid';
                                                $statusIconFile = 'icon_no.png';

                                                if ($isWinner) {
                                                    $statusIconFile = 'icon_yes.png';

                                                    if ($isDelivered) {
                                                        $statusTitle = 'Character delivered';
                                                        $statusColor = '#2f8f2f';
                                                    } elseif ($isAuctionFinished) {
                                                        $statusTitle = 'Awaiting transfer';
                                                        $statusColor = '#b36b00';
                                                    } else {
                                                        $statusTitle = 'Highest bid';
                                                        $statusColor = '#2f8f2f';
                                                    }
                                                } else {
                                                    $statusColor = '#a80000';
                                                ...
                                                $endDateFormatted = $auctionEndTimestamp !== false
                                                    ? date('d M Y', $auctionEndTimestamp)
                                                    : '';
                                                ?>
                                                <tr bgcolor="<?= getStyle($i_bg); ?>">
                                                    <td style="color: <?= $statusColor ?>;"><?= $getCharacterbyAccount['name'] ?></td>
                                                    <td style="color: <?= $statusColor ?>;"><?= $endDateFormatted; ?></td>
                                                    <td style="color: <?= $statusColor ?>;">
                                                        <img
                                                            src="<?= $template_path; ?>/images/charactertrade/<?= $statusIconFile ?>"
                                                            alt="<?= $statusTitle; ?>"
                                                            title="<?= $statusTitle; ?>">
                                                    </td>
                                                    <td style="color: <?= $statusColor ?>;">
                                                        <?= number_format((int) $BidbyAccount['bid'], 0, ',', ','); ?>
                                                        <img
                                                            src="<?= $template_path; ?>/images//account/icon-tibiacointrusted.png"
                                                            class="VSCCoinImages" title="Transferable Tibia Coins">
                                                    </td>
                                                    <?php if (!$isAuctionFinished) { ?>
                                                        <td>
