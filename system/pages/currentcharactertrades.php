...
if ($logged) {
    $getAccountCoins = $db->query("SELECT `id`, `premdays`, `coins_transferable` FROM `accounts` WHERE `id` = {$account_logged->getId()}");
                ...
                <td>
                    <div class="InnerTableContainer">
                        <table style="width:100%;">
                            ...
                            $countStatement = $db->prepare('SELECT COUNT(*) FROM `myaac_charbazaar` WHERE `date_end` >= :limitDate AND `status` = 0');
                            ...
                            $auctionStatement = $db->prepare('SELECT `id`, `account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`, `bid_account`, `bid_price`, `status` FROM `myaac_charbazaar` WHERE `date_end` >= :limitDate AND `status` = 0 ORDER BY `date_start` DESC LIMIT :limit OFFSET :offset');
...
if ($getPageAction == 'bid') {
    ...
    if ($logged && $getAuction['account_old'] != $account_logged) {
        ...
        if ($logged) {
            $getAccount = $db->query('SELECT `id`, `premdays`, `coins_transferable` FROM `accounts` WHERE `id` = ' . $account_logged->getId() . '');
                                    ...
                                    <td>
                                        <div class="TableContentContainer">
                                            <table class="TableContent" style="border:1px solid #faf0d7;" width="100%">
                                                <tbody>
                                                <tr>
                                                    <td style="font-weight:normal;"><?= $getAccount['coins_transferable'] ?>
                                                        <img
...
if ($getPageAction == 'bidfinish') {
    ...
    if (isset($_POST['bid_confirm']) && $_POST['bid_max'] && $logged) {
        ...
        if ($countAuctionBid > 0) {
            ...
            $getAccountOldBid = $db->query('SELECT `id`, `coins_transferable` FROM `accounts` WHERE `id` = ' . $getAuctionBid['account_id'] . '');
            $getAccountOldBid = $getAccountOldBid->fetch();
            $SomaCoinsOldBid = $getAccountOldBid['coins_transferable'] + $getAuctionBid['bid'];
            $UpdateAccountOldBid = $db->exec('UPDATE `accounts` SET `coins_transferable` = ' . $SomaCoinsOldBid . ' WHERE `id` = ' . $getAuctionBid['account_id'] . '');
            // OLD BID ACCOUNT RETURN COINS

            // NEW BID ACCOUNT REMOVE COINS
            $getAccountNewBid = $db->query('SELECT `id`, `coins_transferable` FROM `accounts` WHERE `id` = ' . $account_logged . '');
            ...
            $UpdateAccountNewBid = $db->exec('UPDATE `accounts` SET `coins_transferable` = ' . $TaxCoinsNewBid . ' WHERE `id` = ' . $account_logged . '');
            // NEW BID ACCOUNT REMOVE COINS
            ...
            $SubCoinsNewBid = $getAccountNewBid['coins_transferable'] - $bid_max;
            $TaxCoinsNewBid = $SubCoinsNewBid - $charbazaar_bid; // TAX TO CREATE BID
            $UpdateAccountNewBid = $db->exec('UPDATE `accounts` SET `coins_transferable` = ' . $db->quote($TaxCoinsNewBid) . ' WHERE `id` = ' . $account_logged . '');
            // NEW BID ACCOUNT REMOVE COINS
...
if ($getPageAction == 'finish') {
    if (!$logged || !isset($_POST['auction_iden'])) {
        header('Location: ' . BASE_URL . '?account/manage');
        exit;
    }

    $auctionId = (int) $_POST['auction_iden'];
    if ($auctionId <= 0) {
        header('Location: ' . BASE_URL . '?account/manage');
        exit;
    }
    ...
    $auctionStatement = $db->prepare(
        'SELECT `id`, `account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`, `bid_account`, `status` ' .
        'FROM `myaac_charbazaar` WHERE `id` = :id'
    );
    $auctionStatement->execute([':id' => $auctionId]);
    $getAuction = $auctionStatement->fetch(PDO::FETCH_ASSOC);

    if (!$getAuction || (int) $getAuction['account_old'] !== (int) $account_logged->getId()) {
        header('Location: ' . BASE_URL . '?account/manage');
        exit;
    }

    $bidStatement = $db->prepare(
        'SELECT `id`, `account_id`, `auction_id`, `bid`, `date` FROM `myaac_charbazaar_bid` ' .
        'WHERE `auction_id` = :auction ORDER BY `bid` DESC, `id` DESC LIMIT 1'
    );
    $bidStatement->execute([':auction' => (int) $getAuction['id']]);
    $getBid = $bidStatement->fetch(PDO::FETCH_ASSOC);

    $hasBid = $getBid && (int) $getBid['bid'] > 0 && (int) $getBid['account_id'] > 0;

    $updateAuctionSold = $db->prepare('UPDATE `myaac_charbazaar` SET `status` = 1, `account_new` = :account WHERE `id` = :id AND `status` = 0');
    $updateAuctionCancelled = $db->prepare('UPDATE `myaac_charbazaar` SET `status` = 2, `account_new` = :account, `bid_account` = 0, `bid_price` = 0 WHERE `id` = :id AND `status` = 0');
    $updateSellerCoins = $db->prepare('UPDATE `accounts` SET `coins_transferable` = `coins_transferable` + :coins WHERE `id` = :id');
    $updatePlayerAccount = $db->prepare('UPDATE `players` SET `account_id` = :account WHERE `id` = :player');
    $deleteAuctionBids = $db->prepare('DELETE FROM `myaac_charbazaar_bid` WHERE `auction_id` = :auction');

    try {
        $db->beginTransaction();

        if ($hasBid) {
            $accountNew = (int) $getBid['account_id'];
            $bidAmount = (int) $getBid['bid'];
            $netCoins = (int) round($bidAmount - (($bidAmount / 100) * $charbazaar_tax));
            if ($netCoins < 0) {
                $netCoins = 0;
            }

            $updateAuctionSold->execute([
                ':account' => $accountNew,
                ':id' => (int) $getAuction['id'],
            ]);

            if ($updateAuctionSold->rowCount() === 0) {
                $db->rollBack();
                header('Location: ' . BASE_URL . '?account/manage');
                exit;
            }
            ...
            $updateSellerCoins->execute([
                ':coins' => $netCoins,
                ':id' => (int) $getAuction['account_old'],
            ]);

            $updatePlayerAccount->execute([
                ':account' => $accountNew,
                ':player' => (int) $getAuction['player_id'],
            ]);
        } else {
            $updateAuctionCancelled->execute([
                ':account' => (int) $getAuction['account_old'],
                ':id' => (int) $getAuction['id'],
            ]);

            if ($updateAuctionCancelled->rowCount() === 0) {
                $db->rollBack();
                header('Location: ' . BASE_URL . '?account/manage');
                exit;
            }
            ...
            $deleteAuctionBids->execute([
                ':auction' => (int) $getAuction['id'],
            ]);
        }
