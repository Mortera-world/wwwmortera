...
function renderTierBadge(int $tier, string $additionalClasses = ''): string
{
  $tier = (int) $tier;
  if ($tier <= 0) {
    return '';
  ...
  $className = 'ItemTierBadge';
  if ($additionalClasses !== '') {
    $className .= ' ' . htmlspecialchars(trim($additionalClasses), ENT_QUOTES, 'UTF-8');
  }

  $iconRelativePath = 'images/items/tier' . $tier . '.png';
  $iconAbsolutePath = BASE . $iconRelativePath;

  if (file_exists($iconAbsolutePath)) {
    return '<span class="' . $className . '"><img src="' . $iconRelativePath . '" alt="Tier ' . $tier . '"></span>';
  }

  return '<span class="' . $className . ' ItemTierBadge--text">Tier ' . $tier . '</span>';
}
...
function load_config_lua($filename, &$meta = null)
{
  global $config;
  ...
  if (count($lines) > 0) {
    foreach ($lines as $ln => $line) {
      ...
      if (strpos($line, 'dofile') !== false) {
        ...
        $result = array_merge($result, load_config_lua($config['server_path'] . $tmp[1], $meta));
      } elseif (count($tmp_exp) >= 2) {
        $key = trim($tmp_exp[0]);
        if (0 !== strpos($key, '--')) {
          $rawValue = trim($tmp_exp[1]);
          $value = $rawValue;
          ...
          $meta[$key] = ['raw' => $rawValue];
          if (strpos($rawValue, '--') !== false) {
            $comment = trim(substr($rawValue, strpos($rawValue, '--') + 2));
            if ($comment !== '') {
              $meta[$key]['comment'] = $comment;
            }
          }
...
function getTopFraggers($limit = 5)
{
  global $db, $config;
  ...
  $killTableCandidates = [];
  if ($db->hasTable('player_kills')) {
    $killTableCandidates[] = 'player_kills';
  }
  if ($db->hasTable('player_killers')) {
    $killTableCandidates[] = 'player_killers';
  }

  if (empty($killTableCandidates)) {
    return [];
  }

  $cache = Cache::getInstance();
  $cache_key = 'top_' . $limit . '_fraggers';

  if ($cache->enabled()) {
    $tmp = '';
    if ($cache->fetch($cache_key, $tmp)) {
      $fraggers = unserialize($tmp);
      if (is_array($fraggers)) {
        return $fraggers;
      }
    }
  }
  ...
  if ($db->hasColumn('players', 'lookaddons')) {
    $columns[] = 'players.lookaddons';
  }

  $hiddenIds = config('highscores_ids_hidden');
  if (!is_array($hiddenIds) || empty($hiddenIds)) {
    $hiddenIds = [0];
  }

  $baseConditions = [
    'players.id NOT IN (' . implode(', ', $hiddenIds) . ')',
    'players.' . $deleted . ' = 0',
    'players.group_id < ' . config('highscores_groups_hidden'),
    'players.account_id != 1',
  ];

  $fraggers = [];

  foreach ($killTableCandidates as $killTable) {
    $conditions = $baseConditions;
    $quotedTable = '`' . $killTable . '`';
    $joins = ' INNER JOIN ' . $quotedTable . ' ON ' . $quotedTable . '.`player_id` = players.id';
    $countColumn = $quotedTable . '.`player_id`';

    if (
      $killTable === 'player_killers' &&
      $db->hasTable('killers') &&
      $db->hasColumn('killers', 'final_hit')
    ) {
      $joins .= ' INNER JOIN `killers` ON `killers`.`id` = ' . $quotedTable . '.`kill_id`';
      $conditions[] = '`killers`.`final_hit` = 1';
      $countColumn = '`killers`.`id`';
    }

    $query = $db->query(
      'SELECT ' .
        implode(', ', $columns) .
        ', COUNT(' . $countColumn . ') AS frags' .
        ' FROM players' .
        $joins .
        ' WHERE ' .
        implode(' AND ', $conditions) .
        ' GROUP BY players.id' .
        ' ORDER BY frags DESC, players.name ASC' .
        ' LIMIT ' .
        $limit
    );

    $fraggers = $query->fetchAll();
  ...
  if (empty($fraggers)) {
    if ($cache->enabled()) {
      $cache->set($cache_key, serialize([]), 60);
    }
  ...
  if ($cache->enabled()) {
    ...
    if (!empty($config['lua']) && is_array($config['lua'])) {
      $saveIntervalKeys = [
        'saveInterval',
        'saveinterval',
        'saveIntervalTime',
      ...
      foreach ($saveIntervalKeys as $intervalKey) {
        if (!isset($config['lua'][$intervalKey])) {
          continue;
        }

        $intervalValue = $config['lua'][$intervalKey];
        ...
        // Some distributions expose the save interval in minutes instead of seconds.
        if (
          stripos($intervalKey, 'time') !== false &&
          $intervalSeconds > 0 &&
          $intervalSeconds <= 60
        ) {
          $intervalSeconds *= 60;
        ...
        if ($intervalSeconds > 0) {
          $intervalRaw = '';
          if (!empty($config['lua_meta']) && is_array($config['lua_meta'])) {
            $metaEntry = $config['lua_meta'][$intervalKey] ?? null;
            if (is_array($metaEntry) && isset($metaEntry['raw'])) {
              $intervalRaw = (string) $metaEntry['raw'];
            } elseif (is_string($metaEntry)) {
              $intervalRaw = $metaEntry;
            }
          }

          $intervalRawLower = strtolower($intervalRaw);
          $millisecondsHint = false;
          if ($intervalRawLower !== '') {
            if (
              strpos($intervalRawLower, 'millisecond') !== false ||
              strpos($intervalRawLower, 'milisecond') !== false ||
              preg_match('/\bms\b|\bmsec\b/', $intervalRawLower)
            ) {
              $millisecondsHint = true;
            }
          }

          $secondsHint = false;
          if (!$millisecondsHint && $intervalRawLower !== '') {
            if (strpos($intervalRawLower, 'second') !== false) {
              $secondsHint = true;
            }
          }

          // Some distributions express the save interval in milliseconds.
          if ($intervalSeconds >= 1000 && $intervalSeconds % 1000 === 0) {
            $looksLikeMilliseconds = $millisecondsHint;
            if (!$looksLikeMilliseconds && !$secondsHint && $intervalSeconds >= 86400) {
              $looksLikeMilliseconds = true;
            }

            if ($looksLikeMilliseconds) {
              $intervalSeconds = (int) max(1, $intervalSeconds / 1000);
            }
          }

          $cacheTtl = max($intervalSeconds, 10);
          break;
        }
      }
    }

    $cache->set($cache_key, serialize($fraggers), $cacheTtl);
  }

  return $fraggers;
}

function getTopExperiencePerHourPlayers($limit = 5)
{
  global $db;

  $limit = (int) max(1, $limit);
  $cache = Cache::getInstance();
  $cache_key = 'top_' . $limit . '_experience_daily';
  ...
  $columns = '`p`.`id`, `p`.`name`, `p`.`level`, `p`.`vocation`, `p`.`experience`, `p`.`looktype`';
  if ($db->hasColumn('players', 'lookaddons')) {
    $columns .= ', `p`.`lookaddons`';
  }

  $columns .= ', `p`.`lookhead`, `p`.`lookbody`, `p`.`looklegs`, `p`.`lookfeet`';
  ...
  $use_kv_store = $db->hasTableAndColumns('kv_store', ['key_name', 'value', 'timestamp']);

  $snapshot_fetch = function ($key) use ($db, $cache, $use_kv_store) {
    $data = [];

    if ($use_kv_store) {
      $stmt = $db->prepare('SELECT `value` FROM `kv_store` WHERE `key_name` = :key LIMIT 1');
  ...
  $snapshot_store = function ($key, $data, $ttl = 86400) use ($db, $cache, $use_kv_store) {
    if ($use_kv_store) {
      $stmt = $db->prepare('REPLACE INTO `kv_store` (`key_name`, `timestamp`, `value`) VALUES (:key, :timestamp, :value)');
      $stmt->bindValue(':key', $key, PDO::PARAM_STR);
      $stmt->bindValue(':timestamp', time(), PDO::PARAM_INT);
      $stmt->bindValue(':value', serialize($data), PDO::PARAM_LOB);
      $stmt->execute();
    } elseif ($cache->enabled()) {
      $cache->set($key, serialize($data), $ttl);
    }
  };

  $current_snapshot_key = 'myaac.top_xp_day.snapshot.current';
  $previous_snapshot_key = 'myaac.top_xp_day.snapshot.previous';

  $current_snapshot = $snapshot_fetch($current_snapshot_key);
  $previous_snapshot = $snapshot_fetch($previous_snapshot_key);

  $current_date = date('Y-m-d');

  if (empty($current_snapshot['date']) || $current_snapshot['date'] !== $current_date) {
    if (!empty($current_snapshot)) {
      $previous_snapshot_ttl = $cache->enabled() ? 0 : 86400;
      $snapshot_store($previous_snapshot_key, $current_snapshot, $previous_snapshot_ttl);
    ...
    foreach ($players as $player) {
      ...
      $previous_total = isset($previous_snapshot['players'][$player_id]['experience'])
        ? (int) $previous_snapshot['players'][$player_id]['experience']
        : (int) $player['experience'];

      $snapshot_players[$player_id] = [
        'experience' => (int) $player['experience'],
        'previous_experience' => $previous_total,
      ];
    }
    ...
    $snapshot_store($current_snapshot_key, $current_snapshot);
  }

  $previous_players = $previous_snapshot['players'] ?? [];
  ...
  foreach ($players as $player) {
    $player_id = (int) $player['id'];
    $experience = (int) $player['experience'];
    $previous_experience = isset($previous_players[$player_id]['experience'])
      ? (int) $previous_players[$player_id]['experience']
      : $experience;

    $previous_previous_experience = isset($previous_players[$player_id]['previous_experience'])
      ? (int) $previous_players[$player_id]['previous_experience']
      : $previous_experience;

    $experience_gain = $experience - $previous_experience;
    if ($experience_gain <= 0) {
      continue;
    }

    $player['experience_gain'] = $experience_gain;
    $player['experience_previous_gain'] = max(0, $previous_experience - $previous_previous_experience);

    $ranked_players[] = $player;
  }

  if (empty($ranked_players)) {
    if ($cache->enabled()) {
      $cache->set($cache_key, serialize([]), 60);
    }
  ...
  usort($ranked_players, function ($a, $b) {
    if ($a['experience_gain'] === $b['experience_gain']) {
    ...
    return $b['experience_gain'] <=> $a['experience_gain'];
  });

  $ranked_players = array_slice($ranked_players, 0, $limit);
  ...
  if ($cache->enabled()) {
    $cache->set($cache_key, serialize($ranked_players), 300);
  }
...
function clearCache()
{
  ...
  if ($cache->enabled()) {
    ...
    if ($cache->fetch('config_lua_meta', $tmp)) {
      $cache->delete('config_lua_meta');
    }
...
function getSkillRequiredTries(int $skillLevel, string $skillType, ?int $vocationId = null): int
{
  $skillLevel = max(0, $skillLevel);
  ...
  if ($vocationId !== null) {
    $skillMultiplier = getSkillMultiplierForVocation($skillType, $vocationId);
  }

  $requiredTries = (int) ceil($baseTries * $skillMultiplier * pow(1.1, $skillLevel));

  return max(1, $requiredTries);
}

function getSkillProgressPercent(int $skillLevel, int $skillTries, string $skillType, ?int $vocationId = null): int
{
  if ($skillTries <= 0) {
    return 0;
  }

  $required = getSkillRequiredTries($skillLevel, $skillType, $vocationId);
  if ($required <= 0) {
    return 0;
  }

  $percent = (int) floor(($skillTries / $required) * 100);
...
function getSkillMultiplierForVocation(string $skillType, int $vocationId): float
{
  static $cachedMultipliers = null;

  if ($cachedMultipliers === null) {
    $cachedMultipliers = loadVocationSkillMultipliers();
  }

  if (!isset($cachedMultipliers[$vocationId][$skillType])) {
    return 1.0;
  }

  return $cachedMultipliers[$vocationId][$skillType];
}

function loadVocationSkillMultipliers(): array
{
  static $cache = null;
  ...
  $skillAttributeMap = [
    'axe' => ['skillaxe'],
    'club' => ['skillclub'],
    'dist' => ['skilldistance', 'skilldist'],
    'distance' => ['skilldistance', 'skilldist'],
    'fist' => ['skillfist'],
    'fishing' => ['skillfish', 'skillfishing'],
    'shielding' => ['skillshield', 'skillshielding'],
    'sword' => ['skillsword'],
  ];
  ...
  foreach ($xml->vocation as $vocation) {
    ...
    foreach ($skillAttributeMap as $skillKey => $attributes) {
      foreach ($attributes as $attribute) {
        if (isset($vocation[$attribute])) {
          $value = (float) $vocation[$attribute];
          $cache[$id][$skillKey] = max(0.1, $value / 10.0);
          break;
        }
      }

      if (!isset($cache[$id][$skillKey])) {
        $cache[$id][$skillKey] = 1.0;
      }
    }
  }
